...
import './VideoMovieM.css'; // Ensure the CSS file is created and styled correctly

const VideoBannerM = ({ videobanners }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const videoRef = useRef(null);

  // Filter videobanners for those of type "movie"
  const filteredVideos = videobanners.filter(video => video.type === 'Movie');
